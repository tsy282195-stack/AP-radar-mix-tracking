...
%ASSERT_PLATFORM_COVERS_MEASUREMENTS Check the platform interpolation domain.
first_t = inf; last_t = -inf;
...
% Query the same handles used by conversion, filtering, and truth evaluation.
query = [first_t, last_t];
platform.interp_lat(query);
platform.interp_lon(query);
platform.interp_alt(query);
t0 = min(platform.t_sec); t1 = max(platform.t_sec);
before_s = max(t0 - first_t, 0); after_s = max(last_t - t1, 0);
if before_s > 1e-9 || after_s > 1e-9
    fprintf(['[Platform coverage] Endpoint linear extrapolation: ' ...
        'start=%.6fs, end=%.6fs (limit=%.6fs).\n'], ...
